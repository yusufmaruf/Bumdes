...
class ReportPurchaseUserController extends Controller
{
    /**
        ...
        if ($request->ajax()) {
            ...
            return datatables()
                ...
                ->addColumn('aksi', function ($groupedData) {
                    $firstTransaction = $groupedData->first();
                    return '<a href="' . route('user.reportpurchaseuser.detail', $firstTransaction->tanggal) . '" class="btn btn-sm btn-primary"> <span class="far fa-eye"></span> Detail</a>';
                })
                ->rawColumns(['Pengeluaran', 'tanggal', 'bumdes', 'aksi'])
                ->make(true);
        }
        $data = Bumdes::all();
        return view('layouts.pages.user.ReportPurchase.index', compact('data'));
    }
    ...
    public function create()
    {
        ...
        return view('layouts.pages.user.ReportPurchase.create', compact('data'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $data['category'] = 'pengeluaran';
        Transaction::create($data);
        return redirect()->route('user.reportpurchaseuser.index')->with('success_message_create', 'Data Berhasil Dibuat');
    }
    ...
    public function update(Request $request,  $id)
    {
        ...
        return redirect()->route('user.reportpurchaseuser.index')->with('success_message_update', 'Data Berhasil Diubah');
    }
    ...
    public function destroy($id)
    {
        ...
        return redirect()->route('user.reportpurchaseuser.index')->with('success_message_delete', 'Data Berhasil Dihapus');
    }
    ...
    public function detail($date, Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return datatables()
                ...
                ->addColumn('Pengeluaran', function ($transaction) {
                    return $transaction->total > 0 ? 'Rp. ' . number_format($transaction->total, 0, ',', '.') : 'Rp. 0';
                })
                ->addColumn('aksi', function ($transaction) {
                    $editUrl = route('user.reportpurchaseuser.edit', $transaction->idTransaction);
                    $deleteUrl = route('user.reportpurchaseuser.destroy', $transaction->idTransaction);
        ...
        $data = Bumdes::where('idUser', auth()->user()->id)->get();
        return view('layouts.pages.user.ReportPurchase.detail', compact('data', 'date'));
    }
}
