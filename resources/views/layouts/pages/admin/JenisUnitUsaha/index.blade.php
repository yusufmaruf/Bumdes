...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Admin /</span> Jenis Unit Usaha</h4>
        <!-- DataTable with Buttons -->
        <div class="card">
            <div class="card-datatable table-responsive pt-0">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h5 class="card-title mb-0">Data Jenis Unit Usaha</h5>
    ...
    @includeIf('layouts.pages.admin.JenisUnitUsaha.modal')
@endsection
    ...
    <script>
        ...
        $(document).ready(function() {
            table = $('.table').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('admin.categoryUnitUsaha.index') }}",
                columns: [{
                    data: 'DT_RowIndex',
                }, {
                    data: 'name',
    ...
    <script>
        $(document).ready(function() {
            $('body').on('click', '.tombol-edit', function(e) {
                var id = $(this).data('id');
                $.ajax({
                    url: '{{ route('admin.categoryUnitUsaha.show', ['categoryUnitUsaha' => ':id']) }}'
                        .replace(
                            ':id',
                            id),
                    type: 'GET',
                    success: function(response) {
                        $('#editdata').modal('show');
                        $('#titleedit').val(response.result.name);
                        $('#updatedata').attr('action',
                            '{{ route('admin.categoryUnitUsaha.update', ['categoryUnitUsaha' => ':id']) }}'
                            .replace(':id',
                                id));
